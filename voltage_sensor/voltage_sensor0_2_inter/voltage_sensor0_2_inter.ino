...
unsigned long       sPeriod[maxSampNum]; 
float               frequency(0); 
float               effValue(0);
...
void setup() {
  ...
  frameTimer = sampleTimer = micros();            // Initial timers setup
}
...
void loop() {
  ...
  if ( k == maxSampNum ) {
    ...
    Serial.print(F("Msmt duration: ")); Serial.print( period ); Serial.print(" uS\t");   
    Serial.print(F("One sample: ")); Serial.print( period / maxSampNum ); Serial.println(" uS");
...
void effectiveValue(int samples[], int sampNum, float *frequency, float *effectiveValue) { 
  ...
  if (samples[0] >= 0) {
   ...
   for (int i = 0; i < sampNum; i++) {
      ...
      if ( halveWave != lastHalve ) {         // Zero crossing detection
        zeroCrossing = true;
  ...
  void timer_handle_interrupts(int timer) {
    ...
    if ( k == 0 ) { 
       sPeriod[k] = micros() - prev_time;       
     } else {
       sPeriod[k] = micros() - prev_msm;            
     }
     prev_msm = micros();
